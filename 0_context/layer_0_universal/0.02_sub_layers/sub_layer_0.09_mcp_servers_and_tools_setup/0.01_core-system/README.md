...
- Playwright MCP is **headed by default**. Use `--headless` to force headless. Do not use `--headless=false` (unsupported; can cause startup failure).
- On **WSLg**, headed Chromium can crash unless launched with Wayland/Ozone flags. Most reliable: run Playwright MCP with `--config` and set `launchOptions.args` to include `--ozone-platform=wayland` and `--enable-features=UseOzonePlatform`.
