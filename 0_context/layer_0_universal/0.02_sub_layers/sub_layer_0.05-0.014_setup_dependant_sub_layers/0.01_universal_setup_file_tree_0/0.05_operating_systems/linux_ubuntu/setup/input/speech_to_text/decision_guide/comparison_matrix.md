# Comparison Matrix

**Last Updated**: January 12, 2026

## Platform Support

| Tool | Linux | macOS | Windows | iOS | Android |
|------|-------|-------|---------|-----|---------|
| **WisprFlow** | No | Yes | Yes | Yes | No |
| **Vibe Typer** | Yes | No | Yes | No | No |
| **OpenWhispr** | Yes | Yes | Yes | No | No |
| **Whisper-Dictation** | Yes | No | No | No | No |
| **Linux-Dictation-Project** | Yes | No | No | No | No |
| **BlabbyAI** | Upcoming | No | No | No | No |

## Privacy & Processing

| Tool | Processing | Privacy Level | Offline Capable |
|------|------------|---------------|-----------------|
| **Vibe Typer** | Cloud | Low | No |
| **OpenWhispr** | Check docs | High | Check |
| **Whisper-Dictation** | 100% Local | Full | Yes |
| **Linux-Dictation-Project** | 100% Local | Full | Yes |
| **BlabbyAI** | TBD | TBD | TBD |

## Features

| Tool | System-wide | Hotkey | Voice Commands | AI Features | Auto-format |
|------|-------------|--------|----------------|-------------|-------------|
| **Vibe Typer** | Yes | Yes | No | Yes (rewrite, reply) | Basic |
| **OpenWhispr** | Yes | Yes | Yes | Check | Check |
| **Whisper-Dictation** | Yes | Yes | No | No | No |
| **Linux-Dictation-Project** | Yes | Yes | Yes | No | No |
| **BlabbyAI** | Yes | Yes | TBD | TBD | Yes (claimed) |

## Technical Requirements

| Tool | Open Source | Setup Difficulty | RAM Required | GPU Helpful |
|------|-------------|------------------|--------------|-------------|
| **Vibe Typer** | No | Easy | Low | No |
| **OpenWhispr** | Yes | Easy | Low-Medium | Check |
| **Whisper-Dictation** | Yes | Medium-Hard | 1-10GB* | Yes |
| **Linux-Dictation-Project** | Yes | Hard | 1-10GB* | Yes |
| **BlabbyAI** | No | TBD | TBD | TBD |

*Depends on Whisper model size chosen

## Environment Compatibility

| Tool | GNOME | KDE | XFCE | Wayland | X11 |
|------|-------|-----|------|---------|-----|
| **Vibe Typer** | Yes | Yes | Yes | Check | Yes |
| **OpenWhispr** | Yes | Yes | Yes | Check | Yes |
| **Whisper-Dictation** | Check | **Best** | Check | **Best** | Check |
| **Linux-Dictation-Project** | Check | Check | Likely | Check | Yes |
| **BlabbyAI** | TBD | TBD | TBD | TBD | TBD |

## Cost

| Tool | Price | Free Tier | Limits |
|------|-------|-----------|--------|
| **Vibe Typer** | Check website | Check | Check |
| **OpenWhispr** | Free | Yes | Open source |
| **Whisper-Dictation** | Free | Yes | Open source |
| **Linux-Dictation-Project** | Free | Yes | Open source |
| **BlabbyAI** | TBD | TBD | TBD |

## Summary Scores

| Tool | Ease of Use | Privacy | Features | Flexibility |
|------|-------------|---------|----------|-------------|
| **Vibe Typer** | ★★★★★ | ★★☆☆☆ | ★★★★☆ | ★★★☆☆ |
| **OpenWhispr** | ★★★★☆ | ★★★★☆ | ★★★☆☆ | ★★★★☆ |
| **Whisper-Dictation** | ★★☆☆☆ | ★★★★★ | ★★☆☆☆ | ★★★★★ |
| **Linux-Dictation-Project** | ★★☆☆☆ | ★★★★★ | ★★★★☆ | ★★★★★ |
| **BlabbyAI** | TBD | TBD | TBD | TBD |

## Related

- [Quick Decision Guide](quick_decision.md) - Priority-based recommendations
- [Use Cases](use_cases.md) - Scenario-based recommendations
- [Platform Options](../platform_options/) - Detailed docs for each tool
