...
│   ├── 05_skills/
│   ├── 06_hooks/scripts/
│   └── 07_episodic_memory/{sessions,changes}/
...
| **Episodic memory** | MUST be named `episodic_memory` | `.0agnostic/07_episodic_memory/` (NOT `episodic/`) |
...
- **Entity-scoped resources live in `.0agnostic/`** with numbered subdirectories (01-08+)
...
- `.0agnostic/07_episodic_memory/` is the source of truth — tool dirs receive synced copies
...
- **Migration**: `tools/migrate-sub-layers-to-0agnostic.sh` (migrates old sub_layers into .0agnostic/)
