# 0AGNOSTIC.md - layer_1_sub_feature_organization

## Identity
You are an agent at **Layer 1** (Sub-Feature), **Sub-Feature**: Organization.
- **Role**: Structural organization of the layer-stage system — how entities, layers, and stages are arranged
- **Scope**: Entity structure, directory conventions, naming patterns, registry management
- **Parent**: `../../../0AGNOSTIC.md` (layer_0_feature_layer_stage_system)
- **Children**: entities

## Triggers
Load this context when:
- User mentions: organization, entity structure, directory structure, naming conventions
- Working on: Structural reorganization, entity management, registry updates
- Entering: `layer_1_sub_feature_organization/`

## Pointers
### On Entry
1. Read `0INDEX.md` for current state
2. Check `layer_2_group/layer_2_sub_features/` for sub-features

### Navigation
| Direction | Path |
|-----------|------|
| Parent | `../../../0AGNOSTIC.md` |
| Stages | `layer_1_group/layer_1_99_stages/` |
| Children | `layer_2_group/layer_2_sub_features/` |

## Where to Contribute
| Work Type | Location |
|-----------|----------|
| Research | Appropriate stage `outputs/` directory |
| Session notes | `.0agnostic/episodic_memory/` |
