...
|---|--------|--------|---------|
| 01 | aalang | empty | No AALang agents defined for this stage |
| 02 | aalang_markdown_integration | empty | No integration MDs |
| 03 | auto_memory | empty | No auto-memory config |
| 04 | @import_references | empty | No @imports |
| 05 | skills | empty | No stage-specific skills |
| 06 | agents | empty | No agent definitions |
...
1. `0AGNOSTIC.md` — stage identity, methodology, status (always loaded)
2. `.0agnostic/01_knowledge/` — distilled research summaries (on-demand)
3. `.0agnostic/02_rules/static/` — research output standards
