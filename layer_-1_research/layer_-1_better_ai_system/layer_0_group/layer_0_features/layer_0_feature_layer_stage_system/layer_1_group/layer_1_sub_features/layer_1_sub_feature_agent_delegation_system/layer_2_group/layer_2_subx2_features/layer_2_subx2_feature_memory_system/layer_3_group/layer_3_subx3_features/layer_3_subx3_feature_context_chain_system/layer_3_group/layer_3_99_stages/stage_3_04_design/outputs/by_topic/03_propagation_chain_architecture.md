...
3. **Changes propagate outward**: Edit .0agnostic/ content (Layer 1), update 0AGNOSTIC.md references (Layer 2), run agnostic-sync.sh (Layer 3), agent picks it up (Layer 4).
4. **Traceability**: Every piece of runtime context must be traceable back to its canonical source path and merge tier.
5. **Reproducibility**: Generated files (Layer 3) must be exactly reproducible from Layer 1 + Layer 2 + .1merge rules.
