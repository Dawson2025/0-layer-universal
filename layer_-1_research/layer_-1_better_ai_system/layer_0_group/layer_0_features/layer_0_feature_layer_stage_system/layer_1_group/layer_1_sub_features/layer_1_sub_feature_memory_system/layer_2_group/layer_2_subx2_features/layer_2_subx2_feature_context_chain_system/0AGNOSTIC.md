...
- **Children**: chain_visualization, context_loading (in `layer_3_group/layer_3_subx3_features/`)
...
2. Read stage reports (`stage_2_XX/outputs/stage_report.md`) for status
...
- **Stage reports**: `stage_2_XX/outputs/stage_report.md` — async status updates from stage agents to manager
...
| Sub-layers | `layer_2_group/layer_2_03_sub_layers/` | knowledge, rules, protocols |
...
│   └── hooks/scripts/
├── .claude/{rules/,skills/}  ← Claude Code integration
...
3. Read `.0agnostic/rules/static/` for always-apply rules
4. Read `.0agnostic/rules/dynamic/` for scenario-specific rules
...
| DOWN to children | chain_visualization, context_loading (layer 3) | `layer_3_group/layer_3_subx3_features/` |
...
| A3 | Skills | `.0agnostic/skills/{chain-validate,avenue-check}/` |
...
| A8 | Agnostic System | `.0agnostic/{rules,knowledge,protocols,skills}/` |

### Key Knowledge Documents

Read these from `.0agnostic/knowledge/` for deep context:
...
| Session notes | `.0agnostic/episodic_memory/sessions/` |
| Change tracking | `.0agnostic/episodic_memory/changes/` |
| Knowledge additions | `.0agnostic/knowledge/` |
| New rules | `.0agnostic/rules/{static,dynamic}/` |
| New protocols | `.0agnostic/protocols/` |
...
| Nested depth naming | `layer_0/.../layer_stage_system/NESTED_DEPTH_NAMING.md` | subxN naming (layer_3_subx3, etc.) |
| Stages explained | `layer_0/.../layer_stage_system/STAGES_EXPLAINED.md` | Stage Completeness Rule |

**Full path prefix**: `layer_0/layer_0_04_sub_layers/sub_layer_0_01_knowledge_system/`
