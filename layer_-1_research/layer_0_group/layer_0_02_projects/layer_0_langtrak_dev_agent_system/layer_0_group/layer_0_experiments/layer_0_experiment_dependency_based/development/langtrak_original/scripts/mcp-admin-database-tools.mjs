// resource_id: "905ea17a-94e9-4560-aed9-abe7b434f177"
// resource_type: "document"
// resource_name: "mcp-admin-database-tools"
#!/usr/bin/env node
...
async function registerAndEnterProject(client) {
  ...
  await callTool(client, 'browser_snapshot', {}, 'Login page initial');
  
  // Get snapshot to find element refs
  const snapshot1 = await callTool(client, 'browser_snapshot', {});
  
  // Switch to Sign Up tab using browser_click (more reliable than evaluate)
  await callTool(client, 'browser_evaluate', {
    function: "() => { const tabs = document.querySelectorAll('.tab-button'); return { count: tabs.length, text0: tabs[0]?.textContent, text1: tabs[1]?.textContent }; }",
  }, 'Check tabs');
  
  await callTool(client, 'browser_evaluate', {
    function: "() => { document.querySelectorAll('.tab-button')[1]?.click(); return 'switched'; }",
  }, 'Switch to Sign Up tab');
  
  await callTool(client, 'browser_wait_for', { time: 0.5 });
  
  // Fill registration using proper form submission approach
  await callTool(client, 'browser_evaluate', {
  ...
  await callTool(client, 'browser_snapshot', {}, 'Registration form filled');
  
  // Submit using direct click - more reliable
  console.log('📝 Submitting registration...');
  const submitResult = await callTool(client, 'browser_evaluate', {
    function: "() => { const btn = document.querySelector('#register-tab .form-button'); if (btn) { btn.click(); return 'clicked'; } return 'not-found'; }",
  }, 'Click Create Account');
  
  // Wait for redirect to dashboard (registration auto-logs in per features/auth/registration.py)
  console.log('⏳ Waiting for registration processing...');
  await callTool(client, 'browser_wait_for', { time: 4 });

  // Registration may surface a modal alert (e.g. "Server authentication failed").
  // Dismiss any dialogs so we can continue automation.
  for (let i = 0; i < 3; i += 1) {
    try {
      await callTool(client, 'browser_handle_dialog', { accept: true }, `Dismiss dialog (${i + 1}/3)`);
    } catch {
      break;
    }
  }

  await callTool(client, 'browser_snapshot', {}, 'After registration submit');

  // Ensure we are authenticated. If registration did not establish a server session, sign in instead.
  let isAuthenticated = false;
  try {
    const authResult = await callTool(
      client,
      'browser_evaluate',
      { function: "() => (document.querySelector('.logout-link') ? 'AUTH' : 'NOAUTH')" },
      'Check authentication state'
    );
    const text = authResult.content?.find((part) => part.type === 'text')?.text || '';
    isAuthenticated = text.includes('AUTH');
  } catch {
    isAuthenticated = false;
  }
  if (!isAuthenticated) {
    console.log('⚠️ Registration did not yield authenticated session; attempting email sign-in...');
    await callTool(client, 'browser_navigate', { url: `${BASE}/login` }, 'Return to login for sign-in');
    await callTool(client, 'browser_evaluate', {
      function: `() => {
        ...
        setValue('#email', ${JSON.stringify(user.email)});
        setValue('#password', ${JSON.stringify(user.password)});
        document.querySelector('#login-tab .form-button')?.click();
        return true;
      }`,
    }, 'Submit email sign-in');
    await callTool(client, 'browser_wait_for', { time: 3 }, 'Wait for sign-in redirect');
    for (let i = 0; i < 2; i += 1) {
      try {
        await callTool(client, 'browser_handle_dialog', { accept: true }, `Dismiss dialog after sign-in (${i + 1}/2)`);
      } catch {
        break;
      }
    }
  ...
  const enterRes = await callTool(client, 'browser_evaluate', {
    function: "() => { const btns = Array.from(document.querySelectorAll('a.action-button')); const enter = btns.find(b => b.textContent.includes('Open') || b.textContent.includes('Enter')); if (enter) { enter.click(); return 'clicked'; } return 'not-found'; }",
  });
  const enterVal = (enterRes.content?.[0]?.text || '').trim();
  if (enterVal.includes('not-found')) {
    throw new Error('Project enter button not found - failed to enter project context');
  }
