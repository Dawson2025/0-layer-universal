...
**Key design**: CLAUDE.md contains explicit "read this" instructions for ALL layers, not just Layer 1. The agent is told to: run jq (Layer 2), read SKILL.md files (Layer 3), read .integration.md (Layer 4), AND check .claude/rules/. Every layer has its own trigger.
