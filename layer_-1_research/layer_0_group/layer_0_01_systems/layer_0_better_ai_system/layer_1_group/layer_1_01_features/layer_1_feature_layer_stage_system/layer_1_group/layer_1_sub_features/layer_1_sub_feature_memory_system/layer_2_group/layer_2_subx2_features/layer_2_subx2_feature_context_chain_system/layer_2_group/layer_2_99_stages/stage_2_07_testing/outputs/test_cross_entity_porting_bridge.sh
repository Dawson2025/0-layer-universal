...
echo "=== Test: Cross-Entity Porting Bridge ==="
...
if [ -f "$DOWNSTREAM_BRIDGE" ] && grep -q "dangerously-bypass-approvals-and-sandbox" "$DOWNSTREAM_BRIDGE"; then
  pass "Downstream bridge includes codex max-permission runtime policy"
else
  fail "Downstream bridge missing codex runtime policy"
fi
