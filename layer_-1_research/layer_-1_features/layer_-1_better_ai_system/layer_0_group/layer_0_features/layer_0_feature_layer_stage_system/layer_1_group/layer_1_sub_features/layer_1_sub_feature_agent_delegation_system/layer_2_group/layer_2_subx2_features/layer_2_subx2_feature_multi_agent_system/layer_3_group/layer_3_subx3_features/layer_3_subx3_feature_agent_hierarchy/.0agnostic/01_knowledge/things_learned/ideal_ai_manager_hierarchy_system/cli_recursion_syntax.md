...
for FEATURE in $FEATURES; do
  ...
  # Create L2 handoff
  cat > "layer_3_$FEATURE/manager_handoff_documents/incoming.json" <<EOF
{
  "schemaVersion": "1.0.0",
...
def spawn_l2_feature_manager(feature_name, task):
    ...
    handoff_path = f"layer_3_{feature_name}/manager_handoff_documents/incoming.json"
    with open(handoff_path, "w") as f:
        json.dump(handoff, f, indent=2)
    ...
    result = subprocess.run(
        [
            "claude-code",
            "--working-dir", f"layer_3_{feature_name}",
            "--allowed", "B(cla:*,codex:*)",
...
feature_request = load_handoff("layer_3_auth/manager_handoff_documents/incoming.json")
