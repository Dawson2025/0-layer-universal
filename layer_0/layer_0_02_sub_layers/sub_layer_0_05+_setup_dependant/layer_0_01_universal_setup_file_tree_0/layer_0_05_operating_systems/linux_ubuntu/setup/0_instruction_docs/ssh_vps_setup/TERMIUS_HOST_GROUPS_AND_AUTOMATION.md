...
The setup uses **separate SSH keys for each device** (not a shared key):

| Device | Key Type | Key Identity | Location |
|--------|----------|--------------|----------|
| VPS | ed25519 | `root@ubuntu-4gb-nbg1-1` | `/root/.ssh/id_ed25519` |
| Linux Laptop | ed25519 | `dawson@github` | `~/.ssh/id_ed25519` |
| iPhone/Termius | ed25519 + RSA | `iPhone-Termius` | Generated by Termius app |
| Windows | ed25519 | (user's key) | `%USERPROFILE%\.ssh\id_ed25519` |

### Why Separate Keys?

1. **Security**: If one device is compromised, only revoke that key
2. **Audit**: Know which device accessed which server
3. **Termius auto-generation**: Termius generates its own keys automatically

### Current Authorized Keys on Linux Laptop

```
# VPS can SSH in
ssh-ed25519 ...zIWu4w root@ubuntu-4gb-nbg1-1

# iPhone/Termius can SSH in (3 keys)
ssh-ed25519 ...HIl+ iPhone-Termius
ssh-ed25519 ...Izw2a iPhone-Termius
ssh-rsa ...XOSIaw== iPhone-Termius
```

### Adding New Device Keys

When a new device needs to SSH into a machine:
1. Get the new device's public key
2. Add it to `~/.ssh/authorized_keys` on the target machine
3. For Termius: The app will show its public key under Settings > Keys
