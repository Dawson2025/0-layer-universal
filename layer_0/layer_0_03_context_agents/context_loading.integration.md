...
| Actor | Purpose |
|-------|---------|
| ctx:ContextLoadingStateActor | — |
| ctx:ContextConfidenceStateActor | — |
| ctx:NavigationStateActor | — |
| ctx:DebugContextStateActor | — |
| ctx:LayerInheritanceStateActor | — |
...
```bash
# List all nodes
jq '."@graph"[]."@id"' context_loading_gab.jsonld

# Get a specific mode
jq '."@graph"[] | select(."@id" == "MODE_ID")' context_loading_gab.jsonld

# Get all modes with purposes
jq '."@graph"[] | select(."@type" == "gab:Mode") | {id: ."@id", purpose: .purpose}' context_loading_gab.jsonld
```

---
*Auto-generated by tools/jsonld-to-md.sh from context_loading_gab.jsonld*
