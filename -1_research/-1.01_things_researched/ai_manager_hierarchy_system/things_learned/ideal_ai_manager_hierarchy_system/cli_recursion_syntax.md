...
cd /workspace/layer_1_ecommerce

claude-code \
  --working-dir "layer_1_ecommerce" \
  --allowed "B(cla:*)" \
...
HANDOFF=$(cat layer_1_ecommerce/manager_handoff_documents/incoming.json)
...
def spawn_l1_project_manager(project_name, task):
    ...
    # Write handoff
    handoff_path = f"layer_1_{project_name}/manager_handoff_documents/incoming.json"
    ...
    result = subprocess.run(
        [
            "claude-code",
            "--working-dir", f"layer_1_{project_name}",
...
def spawn_l2_feature_manager(feature_name, task):
    ...
    # Load L1 context
    l1_context = load_context("layer_1_ecommerce/CLAUDE.md")
...
project_request = load_handoff("layer_1_ecommerce/manager_handoff_documents/incoming.json")
...
def spawn_l3_component_worker(component_name, task, tool="codex"):
    ...
    handoff_path = f"layer_3_{component_name}/hand_off_documents/incoming.json"
    with open(handoff_path, "w") as f:
        json.dump(handoff, f, indent=2)
    ...
    if tool == "codex":
        result = subprocess.run(
            [
                "codex",
                "--model", "codestral",
                "--agents-md", f"layer_3_{component_name}/AGENTS.md",
        ...
        result = subprocess.run(
            [
                "claude-code",
                "--working-dir", f"layer_3_{component_name}",
                "--model", "claude-haiku",
                "process-handoff", handoff_path
            ],
            capture_output=True
